...
int main (int argc, char* argv[])
{
	...
	fdiss = (double *)malloc(sizeof(double)*nt);
	readDiss (Tname, fdiss);
	...
	*/
	...
	if (plotMode==0)
	{
		...
		for (i=id; i<nt; i+=p)
		{
			control.nchan = fdiss[i];
			control.chanBW = control.bw/(double)(control.nchan);
			control.scint_freqbw = control.chanBW;
			...
			printf ("%d %lf %lf %f %lf %d\n", control.nchan, control.whiteLevel, noiseStructure.detection, control.cFlux, acfStructure.probability, nMax);
